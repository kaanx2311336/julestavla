...
public sealed class OpenRouterClient
{
    ...
    private static async Task<string> SendTestRequestAsync(ProjectSettings settings, string apiKey, string model, CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            messages = new object[]
            {
                new { role = "user", content = "TavlaJules OpenRouter baglanti testi. Sadece 'baglanti tamam' yaz." }
            },
            temperature = 0,
            max_tokens = 32
        };
    ...
    private static IReadOnlyList<string> GetModelChain(ProjectSettings settings)
    {
        var models = new List<string>();
        AddModel(settings.OpenRouterModel);
